...
def plot_snps_ratios_genome(arguments):
    ...
    for index, chrom in enumerate(chroms):
        if chrom in chroms:
            ...
            if snps_het or snps_homo:
                ...
                ref_start_values = [i for i in range(0, last_snp_pos, arguments['bin_size_snps'])]
    ...
    loh_plots_genome(df_snps_in_csv, pd.concat(df_snps_ratios), arguments)
    ...
    for index, chrom in enumerate(chroms):
        if chrom in chroms:
            ...
            if arguments['tumor_vcf']:
                ...
                centromere_region_starts, centromere_region_ends, loh_region_starts, loh_region_ends, df_snps_ratios_chrom = plot_snps(chrom, index, df_snps_in_csv, html_graphs, arguments, df_chrom)
                df_snps_ratios.append(df_snps_ratios_chrom)
...
def loh_plots_genome(df_snps_in_csv, df_snps_ratios, arguments):
    ...
    indices = np.arange(0, len(df_snps_ratios)*arguments['bin_size_snps'], arguments['bin_size_snps'], dtype=int)
    ...
    if arguments['pdf_enable']:
        print_genome_pdf(fig, arguments['genome_name']+'_loh', arguments['out_dir_plots'])

    fig.write_html(arguments['out_dir_plots'] +'/'+ arguments['genome_name'] + "_genome_loh.html")