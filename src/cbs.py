...
def rsegment(x, start, end, L=[], shuffles=1000, p=.05):
    ...
    log.info(
        'Proposed partition of {} to {} from {} to {} with t value {} is {}'.format(start, end, start + s, start + e, t,
                                                                                    threshold))
...
def validate(x, L, shuffles=1000, p=.01):
    ...
    for test, s in enumerate(S[1:-1]):
        ...
        log.info('Testing validity of {} in interval from {} to {} yields statistic {}'.format(S[test + 1], S[left],
                                                                                               S[test + 2], t))
        threshold = 0
        thresh_count = 0
