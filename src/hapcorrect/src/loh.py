...
def detect_loh_centromere_regions(chrom, arguments, centromere_region_starts, centromere_region_ends, loh_region_starts, loh_region_ends, ref_start_values, ref_end_values, haplotype_1_values, haplotype_2_values, unphased_reads_values, haplotype_1_values_phasesets, haplotype_2_values_phasesets, ref_start_values_phasesets, ref_end_values_phasesets):
...
def plot_snps(arguments, df_snps_in_csv):
    filename = f"{os.path.join(arguments['out_dir_plots'], 'SNPs_DEBUG.html')}"
    ...
    for index, chrom in enumerate(chroms):
        plot_snps_freqs_ratios_counts(chrom, index, df_snps_in_csv, html_graphs, arguments)

    html_graphs.write("</body></html>")

def plot_snps_freqs_ratios_counts(chrom, index, df_snps_in_csv, html_graphs, arguments):
    ...
    plot_snps_counts(chrom, index, ref_start_values, df_snps_in_csv, html_graphs, arguments)
...
def plot_snps_counts(chrom, index, ref_start_values, df_snps_in_csv, html_graphs, arguments):
    fig = go.Figure()
    ...
    print_chromosome_html(fig, chrom + '_snps_counts', html_graphs, arguments['out_dir_plots'])
    html_graphs.write(
